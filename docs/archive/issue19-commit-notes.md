...
- 계급 순서는 `원사 -> 준위 -> 소위` 기준으로 관리합니다.
- Storage에는 `0.png~18.png`뿐 아니라 `default.png`도 반드시 있어야 합니다.
- Storage 파일명은 `0.png`, `default.png`처럼 정확히 맞아야 하며, `0.png.png` 같은 오타가 나면 이미지가 표시되지 않습니다.
