...
1. 소셜 로그인 후 `profiles` 기반으로 서비스 프로필을 관리하도록 흐름을 정리했습니다.
2. 회원 유형을 `일반인 / 현역군인(병) / 현역간부`로 분리하고, 유형별 입력/검증 규칙을 코드에서 분리했습니다.
3. 현역병은 입대일과 복무유형 기준으로 현재 계급과 진행률이 자동 계산되도록 바꿨습니다.
4. 일반인은 지인 1명의 복무 정보를 등록하면 대시보드에서 그 지인 기준 진행률을 볼 수 있습니다.
5. 마이페이지는 조회 전용 화면으로 재구성했고, 개인정보 수정은 별도 모달로 분리했습니다.
6. 마이페이지에 회원탈퇴 위험 구역과 확인 모달을 추가하고, 백엔드 계정 삭제 API까지 연결했습니다.
7. 회원탈퇴 시 프로필, 장바구니, 쿠폰, 북마크, 주문과 연결된 데이터를 정리하고 Auth 계정까지 삭제합니다.
8. 예비군 페이지는 런타임 지오코딩을 제거하고 좌표 포함 정적 JSON 기반 지도 + 전체 검색 구조로 단순화했습니다.
9. 정책/고객지원 페이지, 푸터, 로고, SEO 메타태그 등 서비스 브랜딩과 공통 문구를 실제 서비스 형태로 정리했습니다.
10. 현재 미커밋 작업은 대시보드 실시간 진행률 UX 조정과 일부 신규 미추적 파일 정리입니다.
